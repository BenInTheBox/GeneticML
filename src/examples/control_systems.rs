...
impl Agent for Controller {
    fn new() -> Self {
        ...
        let x_acc_coeff = rand::thread_rng().gen_range(-1.0..=1.0);
    ...
    fn step(&mut self, input: Vec<f64>) -> Vec<f64> {
        let mut a = (self.x_coeff * input[0]) + (self.x_acc_coeff * input[1]) + (self.theta_coeff * input[2]) + (self.theta_dot_coeff * input[3]);
        a =  f64::max(-9., f64::min(a, 9.));
        vec![a]
    }
    ...
    fn mutate(&self, mutation_rate: f64) -> Self {
        ...
        let x_acc_coeff_delta = rand::thread_rng().gen_range(-mutation_rate..=mutation_rate);
        ...
        Controller {
            ...
            x_acc_coeff: self.x_acc_coeff + x_acc_coeff_delta,
...
#[derive(Clone)]
pub struct InvertedPendulum<A: Agent> {
    agent: A,
    m: f64, // Mass of the pendulum
    M: f64, // Mass of the cart
...
impl<A: Agent> Simulation<A> for InvertedPendulum<A> {
    ...
    fn on_generation(&mut self, generation_number: usize) {
        let starting_angle = rand::thread_rng().gen_range(-(PI/8.)..=(PI/8.));
...
pub fn inverted_pendulum_exemple() {
